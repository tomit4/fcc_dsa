 ...
 * digit. Add the two numbers and return the sum as a linked list. You may
 * assume the two numbers do not contain any leading zero, except for the number
 * 0 itself.
 * EXAMPLE:
 * a --> [3, ] --> [4, ] --> [3, ] --> null
 * b --> [5, ] --> [6, ] --> [4, ] --> null
...
public class SinglyLinkedList {
  ...
  public ListNode add(ListNode a, ListNode b) {
    ...
    while (a != null || b != null) {
      ...
      int sum = carry + x + y;
      // NOTE: cool little magic here in these next couple lines
      // we first simply take any amount over ten and reassign carry to it
      carry = sum / 10;
      // we then assign a new node that only takes the remainder of that
      // division
    ...
    if (carry > 0) {
      // any carry that remains at the end is reassigned to a new list node
  ...
  public static void main(String[] args) {
    ...
    ListNode addedList = sll.add(a.head, b.head);
    sll.display(a.head);    // 7 --> 4 --> 9 --> null
    sll.display(b.head);    // 5 --> 6 --> null
    sll.display(addedList); // 2 --> 1 --> 0 --> 1 --> null
  }
}
